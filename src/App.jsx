...
function Team({ws,reload,notify}) {
  ...
  const createStaff=async()=>{try{await api("/staff",{method:"POST",body:staff});notify("Staff account created and assigned to branch");setStaff({displayName:"",email:"",password:"",role:"cashier",branchId:String(ws.branches[0]?.id||"")});setShow(false);await reload();}catch(err){notify(err.message,"error");}};
  ...
  const addBranch=async()=>{try{await api("/branches",{method:"POST",body:branch});notify("Branch created");setBranch({name:"",address:""});await reload();}catch(err){notify(err.message,"error");}};
  const chooseQr=file=>{
    if(!file)return;
  ...
  const saveGcash=async()=>{try{await api("/business/gcash",{method:"POST",body:gcash});notify("GCash scan-to-pay QR updated");await reload();}catch(err){notify(err.message,"error");}};
  ...
  return <div className="stack">
    ...
    <section className="section-bar"><div><h2>People & locations</h2><p>Create staff accounts, assign each person to a branch, and configure GCash scan-to-pay.</p></div><button className="btn primary" onClick={()=>setShow(!show)}><Plus/>Add staff</button></section>
    {show&&<Panel title="Create staff account" sub="Every staff account is assigned to one POS branch."><div className="form-grid"><label>Name<input value={staff.displayName} onChange={e=>setStaff({...staff,displayName:e.target.value})}/></label><label>Email<input type="email" value={staff.email} onChange={e=>setStaff({...staff,email:e.target.value})}/></label><label>Password<input type="password" value={staff.password} onChange={e=>setStaff({...staff,password:e.target.value})}/></label><label>Role<select value={staff.role} onChange={e=>setStaff({...staff,role:e.target.value})}><option value="cashier">Cashier</option><option value="inventory">Inventory</option><option value="manager">Manager</option><option value="admin">Tenant Admin</option></select></label><label>Assigned branch<select value={staff.branchId} onChange={e=>setStaff({...staff,branchId:e.target.value})}>{ws.branches.map(b=><option key={b.id} value={b.id}>{b.name}</option>)}</select></label><button className="btn primary" onClick={createStaff}>Create staff</button></div></Panel>}
    <div className="two-col">
      <Panel title="Team accounts" sub="Branch assignment controls which store a staff member can use in POS"><div className="list team-list">{ws.members.map(m=><div key={m.id}><span><b>{m.display_name}</b><small>{m.email} · {m.role}{m.assigned_branch_name?" · "+m.assigned_branch_name:""}</small></span><span className="row-actions">{m.role!=="owner"&&<select className="staff-branch-select" value={m.branch_id||""} onChange={e=>assignBranch(m,e.target.value)}><option value="" disabled>Select branch</option>{ws.branches.map(b=><option key={b.id} value={b.id}>{b.name}</option>)}</select>}<span className={"badge "+(m.is_active?"completed":"voided")}>{m.is_active?"Active":"Disabled"}</span>{m.role!=="owner"&&<button className="btn secondary small" onClick={()=>toggle(m)}>{m.is_active?"Disable":"Enable"}</button>}</span></div>)}</div></Panel>
      <Panel title="Branches" sub={ws.business.branchLimit+" allowed on "+plans[ws.business.plan].name}><div className="list">{ws.branches.map(b=><div key={b.id}><span><b>{b.name}</b><small>{b.address||"No address set"}</small></span><span className="badge completed">Active</span></div>)}</div><div className="form-stack inset"><label>Name<input value={branch.name} onChange={e=>setBranch({...branch,name:e.target.value})}/></label><label>Address<input value={branch.address} onChange={e=>setBranch({...branch,address:e.target.value})}/></label><button className="btn primary" onClick={addBranch}>Add branch</button></div></Panel>
    </div>
    <Panel title="GCash scan-to-pay" sub="Upload the store's official GCash QR. Customers scan this QR from the POS, then the cashier records the transaction reference."><div className="gcash-settings"><div className="gcash-preview">{gcash.qrData?<img src={gcash.qrData} alt="GCash QR preview"/>:<div><QrCode/><span>No QR uploaded</span></div>}</div><div className="form-stack gcash-form"><label>GCash account name<input value={gcash.accountName} onChange={e=>setGcash({...gcash,accountName:e.target.value})} placeholder="Business / account name"/></label><label>GCash mobile / merchant number<input value={gcash.accountNumber} onChange={e=>setGcash({...gcash,accountNumber:e.target.value})} placeholder="09xx xxx xxxx or merchant ID"/></label><label>QR image<input type="file" accept="image/png,image/jpeg,image/webp" onChange={e=>chooseQr(e.target.files?.[0])}/></label><div className="form-actions">{gcash.qrData&&<button className="btn secondary" onClick={()=>setGcash({...gcash,qrData:""})}>Remove QR</button>}<button className="btn primary" onClick={saveGcash}>Save GCash QR</button></div><div className="info-note"><b>Scan-to-pay:</b> BrewPoint displays this QR at checkout. Automatic payment confirmation requires an official GCash/merchant payment-gateway integration and merchant credentials, so the cashier still records the payment reference in this build.</div></div></div></Panel>
  </div>;
}
