  ...
  return <div className="sales-summary-page">
    ...
    {loading||!report?<div className="dashboard-loading">Building sales summary…</div>:<>
      ...
      <section className="best-sellers-card">
        <div className="sales-table-head"><div><h3>Best sellers</h3><p>Top products by quantity sold for the selected filters.</p></div><button className="btn secondary" onClick={()=>go("reports")}>Open item report</button></div>
        <div className="best-seller-grid">{[...(report.products||[])].sort((x,y)=>Number(y.qty)-Number(x.qty)).slice(0,5).map((p,i)=><article key={String(p.id)+p.name}><span className="best-rank">#{i+1}</span><div><b>{p.name}</b><small>{p.category} · {Number(p.qty).toLocaleString()} sold</small></div><strong>{money(p.netSales)}</strong></article>)}{!(report.products||[]).length&&<Empty Icon={ShoppingBag} text="No product sales in this period yet."/>}</div>
      </section>
...
function Inventory({ws,reload,notify}) {
  ...
  const save=async()=>{if(ingredient.name.trim().length<2)return notify("Ingredient name is required.","error");if(!ingredient.uom.trim())return notify("Unit of measure is required.","error");if(Number(ingredient.stockQty)<0||Number(ingredient.lowStockThreshold)<0||Number(ingredient.costPerUnit)<0)return notify("Stock, low-stock level, and cost cannot be negative.","error");try{await api("/ingredients/save",{method:"POST",body:{id:editingId||undefined,...ingredient,stockQty:Number(ingredient.stockQty),lowStockThreshold:Number(ingredient.lowStockThreshold),costPerUnit:Number(ingredient.costPerUnit)}});notify(editingId?"Ingredient updated":"Ingredient saved");resetIngredient();await reload();}catch(err){notify(err.message,"error");}};
...
function Products({ws,reload,notify}) {
  ...
  const save=async()=>{if(form.name.trim().length<2)return notify("Product name is required.","error");if(!form.categoryId)return notify("Select a product category.","error");if(!Number(form.price)||Number(form.price)<=0)return notify("Selling price must be greater than zero.","error");try{await api("/products/save",{method:"POST",body:{id:editing?.id,name:form.name,categoryId:form.categoryId,price:Number(form.price),sku:form.sku,recipes:form.recipes}});notify(editing?"Product recipe & costing updated":"Product added");reset();await reload();}catch(err){notify(err.message,"error");}};
  ...
  const addCategory=async()=>{if(categoryName.trim().length<2)return notify("Category name is required.","error");try{const r=await api("/categories",{method:"POST",body:{name:categoryName}});notify("Category created");setCategoryName("");await reload();setForm(cur=>({...cur,categoryId:String(r.id)}));}catch(err){notify(err.message,"error");}};
    ...
    <div className="two-col product-costing-layout">
      ...
      <Panel title={editing?"Recipe & costing editor":"Add product & recipe"} sub={editing?"Editing "+editing.name:"Build the recipe before saving"}>
        <div className="form-stack">
          ...
          <div className="form-actions">{editing&&<button className="btn secondary" onClick={reset}>Cancel</button>}<button className="btn primary" disabled={form.name.trim().length<2||!form.categoryId||!Number(form.price)||Number(form.price)<=0} onClick={save}>{editing?"Save recipe & costing":"Add product"}</button></div>
        </div>
      </Panel>
    </div>
  </div>;
...
function Reports({ws,reload,notify}) {
  ...
  const inventoryRows=useMemo(()=>ws.ingredients.map(i=>{
    const movements=ws.inventoryMovements.filter(m=>String(m.ingredient_id)===String(i.id));
    ...
    return {...i,stock,threshold,cost,status:stock<=0?"Out of stock":stock<=threshold?"Low stock":"Healthy",value:stock*cost,lastMovement:last?.created_at||null,lastMovementType:last?.movement_type||"—"};
  }).sort((x,y)=>(x.status==="Out of stock"?0:x.status==="Low stock"?1:2)-(y.status==="Out of stock"?0:y.status==="Low stock"?1:2)||x.name.localeCompare(y.name)),[ws.ingredients,ws.inventoryMovements]);

  const inventorySummary={
    active:inventoryRows.filter(i=>i.is_active).length,
    low:inventoryRows.filter(i=>i.is_active&&i.stock<=i.threshold).length,
    out:inventoryRows.filter(i=>i.is_active&&i.stock<=0).length,
    value:inventoryRows.reduce((s,i)=>s+i.value,0)
  };
  ...
  const summaryReceipts=summaryDetail&&report?report.transactions.filter(s=>dayKey(s.created_at)===summaryDetail.date):[];
  ...
  const reportMatrix=()=>({
    ...
    inventory:{title:"Inventory report",headers:["Ingredient","UOM","Current stock","Low-stock level","Status","Cost / unit","Inventory value","Last movement","Movement type"],rows:inventoryRows.map(i=>[i.name,i.uom,Number(i.stock).toLocaleString(),Number(i.threshold).toLocaleString(),i.status,money(i.cost),money(i.value),i.lastMovement?dateTime(i.lastMovement):"—",String(i.lastMovementType).replaceAll("_"," ")])}
  })[view];

  const exportRows=()=>{if(!report)return;const pick=reportMatrix();const csv=[pick.headers,...pick.rows].map(r=>r.map(c=>'"'+String(c??"").replaceAll('"','""')+'"').join(",")).join("\n");const url=URL.createObjectURL(new Blob([csv],{type:"text/csv"}));const el=document.createElement("a");el.href=url;el.download="BrewPoint-"+view+"-"+(view==="inventory"?"current":period)+".csv";el.click();URL.revokeObjectURL(url);};

  const printReport=()=>{
    const pick=reportMatrix();
    const w=window.open("","_blank","width=1100,height=760");
    if(!w)return notify("Allow pop-ups to print the report.","warn");
    const esc=v=>String(v??"").replace(/[&<>"]/g,m=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;"}[m]));
    const summary=view==="inventory"
      ? `<div class="summary"><div>Ingredients<b>${inventorySummary.active}</b></div><div>Low / out<b>${inventorySummary.low}</b></div><div>Inventory value<b>${esc(money(inventorySummary.value))}</b></div></div>`
      : `<div class="summary"><div>Gross sales<b>${esc(money(report.grossSales))}</b></div><div>Net sales<b>${esc(money(report.netSales))}</b></div><div>Gross profit<b>${esc(money(report.grossProfit))}</b></div></div>`;
    w.document.write(`<!doctype html><html><head><title>BrewPoint - ${esc(pick.title)}</title><style>body{font-family:Arial,sans-serif;color:#10253d;padding:28px}h1{margin:0}p{color:#667;margin:6px 0 22px}.summary{display:flex;gap:20px;margin:18px 0}.summary div{border:1px solid #ddd;padding:10px 14px;border-radius:8px}.summary b{display:block;font-size:18px;margin-top:4px}table{width:100%;border-collapse:collapse;font-size:11px}th,td{border-bottom:1px solid #ddd;padding:8px;text-align:left}th{background:#f6f3ee}@media print{button{display:none}}</style></head><body><h1>BrewPoint · ${esc(pick.title)}</h1><p>${esc(ws.business.name)} · ${view==="inventory"?"Current stock snapshot":esc(period)} · Printed ${esc(new Date().toLocaleString())}</p>${summary}<table><thead><tr>${pick.headers.map(h=>"<th>"+esc(h)+"</th>").join("")}</tr></thead><tbody>${pick.rows.map(r=>"<tr>"+r.map(c=>"<td>"+esc(c)+"</td>").join("")+"</tr>").join("")}</tbody></table><script>window.onload=()=>window.print()<\/script></body></html>`);
    w.document.close();
  };

  const printDaySummary=row=>{
    const receipts=report.transactions.filter(s=>dayKey(s.created_at)===row.date);
    const w=window.open("","_blank","width=980,height=760");
    if(!w)return notify("Allow pop-ups to reprint the sales summary.","warn");
    const esc=v=>String(v??"").replace(/[&<>"]/g,m=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;"}[m]));
    w.document.write(`<!doctype html><html><head><title>BrewPoint Sales Summary ${esc(row.date)}</title><style>body{font-family:Arial,sans-serif;color:#10253d;padding:30px}h1{margin:0}.meta{color:#667;margin:6px 0 20px}.kpis{display:grid;grid-template-columns:repeat(3,1fr);gap:10px;margin:18px 0}.kpis div{border:1px solid #ddd;border-radius:8px;padding:10px}.kpis span,.kpis b{display:block}.kpis span{font-size:10px;color:#667}.kpis b{font-size:18px;margin-top:4px}table{width:100%;border-collapse:collapse;margin-top:18px;font-size:11px}th,td{padding:8px;border-bottom:1px solid #ddd;text-align:left}th{background:#f6f3ee}</style></head><body><h1>BrewPoint · Daily Sales Summary</h1><div class="meta">${esc(ws.business.name)} · ${esc(row.date)} · Reprinted ${esc(new Date().toLocaleString())}</div><div class="kpis"><div><span>Gross sales</span><b>${esc(money(row.grossSales))}</b></div><div><span>Discounts</span><b>${esc(money(row.discounts))}</b></div><div><span>Net sales</span><b>${esc(money(row.netSales))}</b></div><div><span>Refunds / voids</span><b>${esc(money(row.refunds))}</b></div><div><span>COGS</span><b>${esc(money(row.cogs))}</b></div><div><span>Gross profit</span><b>${esc(money(row.grossProfit))}</b></div></div><h3>Receipts</h3><table><thead><tr><th>Reference</th><th>Time</th><th>Branch</th><th>Employee</th><th>Payment</th><th>Status</th><th>Total</th></tr></thead><tbody>${receipts.map(s=>"<tr><td>"+esc(s.reference_no)+"</td><td>"+esc(dateTime(s.created_at))+"</td><td>"+esc(s.branch_name)+"</td><td>"+esc(s.cashier_name||"—")+"</td><td>"+esc(String(s.payment_method).toUpperCase())+"</td><td>"+esc(s.status)+"</td><td>"+esc(money(s.total))+"</td></tr>").join("")}</tbody></table><script>window.onload=()=>window.print()<\/script></body></html>`);
    w.document.close();
  };
  ...
  const printReceipt=detail=>{
    ...
    const w=window.open("","_blank","width=520,height=760");
    if(!w)return notify("Allow pop-ups to reprint the invoice.","warn");
    const esc=v=>String(v??"").replace(/[&<>"]/g,m=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;"}[m]));
    w.document.write(`<!doctype html><html><head><title>${esc(sale.reference_no)}</title><style>body{font-family:Arial,sans-serif;color:#10253d;padding:24px;max-width:480px;margin:auto}h1{margin:0;font-size:24px}.meta{font-size:11px;color:#667;line-height:1.6;margin:7px 0 18px}.line,.total{display:flex;justify-content:space-between;gap:14px;padding:8px 0;border-bottom:1px dashed #ccc}.line span{font-size:11px}.line small{display:block;color:#777;margin-top:2px}.totals{margin-top:12px}.total.grand{font-size:18px;font-weight:700;border-top:2px solid #10253d}.foot{text-align:center;font-size:10px;color:#777;margin-top:24px}</style></head><body><h1>BrewPoint</h1><div class="meta">${esc(ws.business.name)}<br>Receipt ${esc(sale.reference_no)}<br>${esc(dateTime(sale.created_at))}<br>${esc(sale.branch_name)} · ${esc(sale.cashier_name||"—")}</div>${items.map(i=>'<div class="line"><span><b>'+esc(i.product_name)+'</b><small>'+esc(Number(i.qty).toLocaleString())+' × '+esc(money(i.unit_price))+'</small></span><b>'+esc(money(i.line_total))+'</b></div>').join("")}<div class="totals"><div class="total"><span>Subtotal</span><b>${esc(money(sale.subtotal))}</b></div><div class="total"><span>Discount</span><b>-${esc(money(sale.discount))}</b></div><div class="total grand"><span>Total</span><b>${esc(money(sale.total))}</b></div>${sale.payment_method==="cash"?'<div class="total"><span>Tendered</span><b>'+esc(money(sale.tendered))+'</b></div><div class="total"><span>Change</span><b>'+esc(money(sale.change_due))+'</b></div>':""}</div><div class="foot">Payment: ${esc(String(sale.payment_method).toUpperCase())}${sale.payment_reference?" · Ref "+esc(sale.payment_reference):""}<br>Reprinted from BrewPoint</div><script>window.onload=()=>window.print()<\/script></body></html>`);
    w.document.close();
  };

  if(!report)return <Loading text="Building reports…"/>;
  const tabs=[["summary","Sales summary"],["items","Sales by item"],["categories","By category"],["inventory","Inventory"],["receipts","Receipts"],["employees","By employee"],["payments","Payment types"]];

  return <div className="stack">
    <section className="section-bar"><div><h2>Reports</h2><p>Sales, receipts, employees, payment types, and a printable current inventory snapshot.</p></div><div className="bar-actions report-filter-actions"><select value={period} disabled={view==="inventory"} onChange={e=>setPeriod(e.target.value)}><option value="daily">Today</option><option value="weekly">This week</option><option value="monthly">This month</option><option value="yearly">This year</option></select><select value={branchId} disabled={view==="inventory"} onChange={e=>setBranchId(e.target.value)}><option value="">All stores</option>{ws.branches.map(b=><option key={b.id} value={b.id}>{b.name}</option>)}</select><select value={employeeId} disabled={view==="inventory"} onChange={e=>setEmployeeId(e.target.value)}><option value="">All employees</option>{ws.members.filter(m=>m.is_active).map(m=><option key={m.user_id} value={m.user_id}>{m.display_name}</option>)}</select><button className="btn secondary" onClick={printReport}><Printer/>Print report</button><button className="btn secondary" onClick={exportRows}>Export CSV</button></div></section>
    <div className="report-tabs">{tabs.map(([id,label])=><button key={id} className={view===id?"active":""} onClick={()=>{setView(id);if(id!=="summary")setSummaryDetail(null)}}>{label}</button>)}</div>

    {view==="inventory"?<div className="metrics report-metrics"><Metric label="Ingredients" value={inventorySummary.active} sub="Active inventory items"/><Metric label="Low stock" value={inventorySummary.low} sub={inventorySummary.out+" out of stock"}/><Metric label="Inventory value" value={money(inventorySummary.value)} sub="Current stock × cost/unit"/><Metric label="Movements" value={ws.inventoryMovements.length} sub="Recent movement records loaded"/></div>:<div className="metrics report-metrics"><Metric label="Gross sales" value={money(report.grossSales)} sub={report.orders+" completed receipts"}/><Metric label="Discounts" value={money(report.discounts)} sub="Applied discounts"/><Metric label="Net sales" value={money(report.netSales)} sub={"Cash "+money(report.cashSales)+" · GCash "+money(report.gcashSales)}/><Metric label="Gross profit" value={money(report.grossProfit)} sub={"COGS "+money(report.estimatedCogs)}/></div>}

    {view==="summary"&&<>
      {summaryDetail&&<Panel title={"Sales summary · "+summaryDetail.date} sub="Open receipts below or reprint this exact daily summary."><div className="summary-detail-metrics"><Metric label="Gross sales" value={money(summaryDetail.grossSales)} sub={summaryDetail.orders+" completed receipts"}/><Metric label="Discounts" value={money(summaryDetail.discounts)} sub="Applied"/><Metric label="Net sales" value={money(summaryDetail.netSales)} sub="After discounts"/><Metric label="Gross profit" value={money(summaryDetail.grossProfit)} sub={"COGS "+money(summaryDetail.cogs)}/></div><div className="table-scroll"><table className="report-table"><thead><tr><th>Reference</th><th>Time</th><th>Branch</th><th>Employee</th><th>Payment</th><th>Status</th><th>Total</th><th>Action</th></tr></thead><tbody>{summaryReceipts.map(s=><tr key={s.id}><td><b>{s.reference_no}</b></td><td>{dateTime(s.created_at)}</td><td>{s.branch_name}</td><td>{s.cashier_name||"—"}</td><td>{String(s.payment_method).toUpperCase()}</td><td>{s.status}</td><td><b>{money(s.total)}</b></td><td><button className="btn secondary tiny" onClick={()=>{setView("receipts");openReceipt(s)}}>View receipt</button></td></tr>)}</tbody></table></div><div className="panel-footer"><button className="btn secondary" onClick={()=>setSummaryDetail(null)}>Close view</button><button className="btn primary" onClick={()=>printDaySummary(summaryDetail)}><Printer/>Reprint summary</button></div></Panel>}
      <Panel title="Sales summary" sub="Daily totals · use View to inspect and reprint a specific day"><div className="table-scroll"><table className="report-table"><thead><tr><th>Date</th><th>Gross sales</th><th>Refunds / voids</th><th>Discounts</th><th>Net sales</th><th>COGS</th><th>Gross profit</th><th>Action</th></tr></thead><tbody>{[...report.breakdown].reverse().map(r=><tr key={r.date}><td>{r.date}</td><td>{money(r.grossSales)}</td><td>{money(r.refunds)}</td><td>{money(r.discounts)}</td><td>{money(r.netSales)}</td><td>{money(r.cogs)}</td><td><b>{money(r.grossProfit)}</b></td><td><button className="btn secondary tiny" onClick={()=>setSummaryDetail(r)}>View</button></td></tr>)}</tbody></table></div></Panel>
    </>}
    ...
    {view==="inventory"&&<Panel title="Inventory report" sub="Current stock snapshot; sales period filters do not change this view"><div className="table-scroll"><table className="report-table inventory-report-table"><thead><tr><th>Ingredient</th><th>UOM</th><th>Current stock</th><th>Low-stock level</th><th>Status</th><th>Cost / unit</th><th>Inventory value</th><th>Last movement</th><th>Movement type</th></tr></thead><tbody>{inventoryRows.map(i=><tr key={i.id}><td><b>{i.name}</b></td><td>{i.uom}</td><td>{Number(i.stock).toLocaleString()}</td><td>{Number(i.threshold).toLocaleString()}</td><td><span className={"inventory-status "+(i.status==="Healthy"?"healthy":i.status==="Low stock"?"low":"out")}>{i.status}</span></td><td>{money(i.cost)}</td><td><b>{money(i.value)}</b></td><td>{i.lastMovement?dateTime(i.lastMovement):"—"}</td><td>{String(i.lastMovementType).replaceAll("_"," ")}</td></tr>)}</tbody></table></div></Panel>}
    ...
    {view==="receipts"&&<>
      {receiptDetail&&<Panel title={"Receipt "+receiptDetail.sale.reference_no} sub={dateTime(receiptDetail.sale.created_at)+" · "+receiptDetail.sale.branch_name}>
        ...
        <div className="panel-footer"><button className="btn secondary" onClick={()=>setReceiptDetail(null)}>Close receipt</button><button className="btn primary" onClick={()=>printReceipt(receiptDetail)}><Printer/>Reprint invoice</button>{receiptDetail.sale.status==="completed"&&<button className="btn danger" onClick={()=>setVoiding(receiptDetail.sale)}>Void receipt</button>}</div>
      </Panel>}
